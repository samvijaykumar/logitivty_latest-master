...
var termsArray;
class UpdateBusinessProfile extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            responseDataList: [],
    ...
    componentDidMount() {
        ...
        this.props.eventsProps.getBussinessApi(data);
    }
            ...
            if (
                ...
                if (response.statusCode === 200) {
                    ...
                    await this.setState({
                        ...
                        // online_business_url: response.data.online_bussiness_url,
                        business_category: response.data.bussiness_category,
                        // google_location: response.data.google_location,

                        // business_address: response.data.bussiness_address,
                        // store_timing: response.data.store_timing,

                        // online_bussiness_url: moment(response.data.start_date).format('YYYY-MM-DD'),
                        // end_date: moment(response.data.end_date).format('YYYY-MM-DD'),
                        // currentterms: response.data.terms_of_use.length
                    });

                    if (response.data.bussiness_type == 'product') {
                        await this.setState({
                            selectbusinesstype: 0,
                        });
                    } else {
                    ...
                    if (response.data.bussiness_mode == 'physical') {
                        await this.setState({
                            selectbusinessmode: 0,
                            google_location: response.data.google_location,
                            business_address: response.data.bussiness_address,
                            store_timing: response.data.store_timing,
                            customer_support_physical: response.data.customer_support_number,
                        });

                        
                    } else {
                        await this.setState({
                            selectbusinessmode: 1,
                            customer_support_online: response.data.customer_support_number,
                            online_business_url: response.data.online_bussiness_url,
                        });
                    }
    ...
    async callApi() {
        ...
        //     AppUtils.showAlert('Please enter Google Location')
        // }
            ...
            // if (this.state.uri != '') {
                ...
                if (this.state.selectbusinessmode === 1) {
                    ...
                    data.append('google_location', '');
                    data.append('bussiness_address', '');
                    data.append('store_timing', '');
                }

                console.log("data: ", data)

                this.props.eventsProps.updateBussinessApi(data);
            // }
            // else {
            //     this.setState({
            //         loader: false
...
{this.props.eventsProps.loading === false ?
                ...
                <ScrollView style={{ backgroundColor: AppColors.colorWhite }}>
                    ...
                    <View style={{ margin: 15 }}>
                        ...
                        <View >
                            ...
                            <Text style={{ marginLeft: wp('2%'), fontFamily: ResourceUtils.fonts.poppins_medium, marginBottom: wp('1%'), marginTop: wp('4%') }}>business mode</Text>
...
const UpdateBusinessProfileElements = connectWithContext(
    MamographyContextProvider
)({
    eventsProps: MamographyContextConsumer,
})(UpdateBusinessProfile);

export default UpdateBusinessProfileElements;
