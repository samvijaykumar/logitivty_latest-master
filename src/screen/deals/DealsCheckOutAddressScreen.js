...
import CheckoutStepView from "../ecommerce/CheckoutStepView";
...
class DealsCheckOutAddressScreen extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      addressList: [],
  ...
  async componentDidUpdate(prevs, prevState, snapshot) {
    ...
    if (
      ...
      if (response.statusCode == 200) {
        // if (AppUtils.isEmpty(this.props.cartProps.responseCartData.data.cart_items)) {
        await this.setState({
          cartList: this.props.cartProps.responseCartData.data,
        });
        // } else {
        //   AppUtils.showAlert(AppStrings.alert_product_sold_out)
        // }
      } else {
      }
    }
    ...
    if (
      prevs.cartProps.loadingCheckDealCart !==
        this.props.cartProps.loadingCheckDealCart &&
      !this.props.cartProps.loadingCheckDealCart
    ) {
      let response = this.props.cartProps.responseCheckDealCart;
      console.log(`check cart deals avail res: ${JSON.stringify(response)}`);
      if (response.statusCode == 200) {
        let isUpdateInCart =
          this.props.cartProps.responseCheckDealCart.data.is_cart_altered;
        if (isUpdateInCart == 1) {
          AppUtils.showAlert(response.message);
  ...
  redirectToHomeSubs = () => {
    ...
    this.props.navigation.navigate("DealsProductListScreen", {
  ...
  checkCartDataAvailability = () => {
    this.props.cartProps.checkDealsCartApiCall({});
  };
  render() {
    const {
      showConfirmDialog,
      ...
      <MenuProvider>
        <View style={{ flex: 1, backgroundColor: "#ffffff" }}>
          ...
          >
            ...
            {AppUtils.isEmpty(addressList) ? //   color={AppColors.colorBlack} //   text={'No address found'} // <NoDataFoundView
            // />
            null : (
              <View
                style={{
                  ...
                  {isVisiblePayment ? (
                    ...
                    >
                      ...
                      >
                        ...
                        />
                        {isVisiblePayment && selectedDeliveryOptionType == 2 ? (
                          <View
                            style={{
                              flexDirection: "row",
                              justifyContent: "space-between",
              ...
              {isVisiblePayment ? (
                ...
                >
                  ...
                  <ButtonView
                    ...
                    onPress={() => {
                      // this.callProceedToPayment();
                      this.checkCartDataAvailability();
                    }}
...
const CheckoutScreenElement = connectWithContext(
  ...
  bookAppointmentProps: DealsContextConsumer,
