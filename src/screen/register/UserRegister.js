...
import DOBPickerDialogeNew from '../../widgets/DOBPickerDialogeNew';
...
class UserRegister extends React.Component {
  ...
  // };
  setDate = (event, dateData) => {
    // date = AppUtils.formatedDateToDatePickerDate(date);
    if (event.type == 'set') {
      this.setState({
        isDialogVisible: false,
        date: dateData,
        dob: AppUtils.datePickerToFormatedDate(dateData),
      });
    } else {
      this.setState({
        isDialogVisible: false,
    ...
    if (
      ...
      if (response.statusCode == 200) {
        this.props.navigation.navigate('OtpNumberScreen', {
          ...
          verify_type:
            response.data.is_account_deleted == 1 ? 'deleted' : 'unverified',
          role_type: 'user',
        });
      }
    }
      ...
      if (response.statusCode == 200) {
        if (response.data.length > 0) {
          ...
          this.setState({
            ...
            staticStateId: response.data[0].id,
          });
          var data = {
      ...
      if (response.statusCode == 200) {
        if (response.data.length > 0) {
          this.setState({
            ...
            staticCityId: response.data[0].id,
          });
          // let tempCityList = [];
              ...
              {/* <View style={{ marginTop: 20 }}>
                ...
                <View style={styles.inputView}>
                  <TextInput
                    ...
                    placeholder={AppStrings.ref_code}
                    placeholderTextColor={AppColors.editTextPlaceHolderColor}
                    returnKeyType="done"
                    keyboardType={"default"}
  ...
  signInAPICalled() {
    ...
    } else if (AppUtils.isNull(_email)) {
      AppUtils.showAlert('Please enter Email.');
    } else if (!this.isValidEmail(eml)) {
      AppUtils.showAlert('Invalid Email. Please enter a valid Email.');
    ...
    // } 
      ...
      if (!AppUtils.isNull(eml) && !AppUtils.isValidEmail(eml)) {
        ...
        console.log('register body param : ', data);
        this.props.userProps.registerUserWithOTP(data);
      }
    }
  }
}
...
const UserRegisterScreenElement = connectWithContext(
...
export default UserRegisterScreenElement;
