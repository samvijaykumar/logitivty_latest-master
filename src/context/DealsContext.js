...
import { getDashboardList, getHomeData, getProductBannersData, getProfileData, getProductCategoriesData, getProductData, 
  ...
  cancelEcomOrderApiCall, savePaymentEcomData, getEComOrderHistoryList,getOrderDetailsApiCall, getDealsBanner, getDealsProductData, getDealsCartData, getDealsCartCountData, addDealCartData, removeDealCartData, dealsCartCheckoutApi, getDealsProductDetails, savePaymentDealsData, cancelDealsOrderApiCall, checkDealsCartApi } from '../network/NetworkCall'

import ResponseParser from '../network/ResponseParser'
export default class DealsContextProvider extends React.Component {
  constructor(props) {
    super(props);
  ...
  get initialState() {
    return {
      ...
      loadingBanner:false,
      bannerResponse:'',
      loadingCheckDealCart:false,
      responseCheckDealCart:''
    };
  }
  ...
  getDashboardProductApi = (data) => {
    ...
    getDashobardProductData(data)
      .then((result) => {
        ...
        this.setState({ errorMessage: '', loadingDashboardProduct: false, dashboardProdResponse: ResponseParser.isResponseOk(response, true) ? response : '' });
      }).catch((error) => {
  ...
  getProductApi = (data) => {
    ...
    getDealsProductData(data)
      .then((result) => {
        const response = result.data;
        this.setState({ errorMessage: '', loadingProduct: false, productResponse: response });
  ...
  getDealBannerApi = () => {
    this.setState({ errorMessage: '', loadingBanner: true, bannerResponse: '' });
    getDealsBanner()
      .then((result) => {
        const response = result.data;
        console.log(`getDealBannerApiSucc: ${JSON.stringify(response)}`)
        this.setState({ errorMessage: '', loadingBanner: false, bannerResponse: ResponseParser.isResponseOk(response, false) ? response : '' });
      }).catch((error) => {
        console.log(`getDealBannerApiErr ${error}`)
        this.setState({ errorMessage: '' + error, loadingBanner: false,bannerResponse:'' });
      });
  };
  checkDealsCartApiCall = () => {
    this.setState({ errorMessage: '', loadingCheckDealCart: true, responseCheckDealCart: '' });
    checkDealsCartApi()
      .then((result) => {
        const response = result.data;
        console.log(`checkDealsCartApiSucc: ${JSON.stringify(response)}`)
        this.setState({ errorMessage: '', loadingCheckDealCart: false, responseCheckDealCart: ResponseParser.isResponseOk(response, false) ? response : '' });
      }).catch((error) => {
        console.log(`checkDealsCartApiErr ${error}`)
        this.setState({ errorMessage: '' + error, loadingCheckDealCart: false,responseCheckDealCart:'' });
      });
  };
  ...
  getState = () => {
    return {
      ...
      getDealBannerApi: this.getDealBannerApi,
      checkDealsCartApiCall: this.checkDealsCartApiCall
    };
  };
...
export const DealsContextConsumer = Context.Consumer;
